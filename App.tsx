...
export default function App() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) throw error;
      if (data.user) {
         // Fallback manual insertion if trigger fails or delayed
         ...
         if (dbError) { console.error("Erro ao salvar perfil (fallback):", dbError); }
  ...
  const fetchOrders = async () => {
      ...
      if (orderData) {
          if (orders.length > 0 && orderData.length === orders.length) {
              // Basic check for status update notification
              const changedOrder = orderData.find(o => {
                  const old = orders.find(oldO => oldO.id === o.id);
                  return old && old.status !== o.status;
              });
              if (changedOrder) {
                  showToast(`Status do pedido atualizado: ${changedOrder.status}`);
              }
          }
          setOrders(orderData);
      }
  ...
  const handleFinalize = async () => {
      ...
      if (!shippingCost) return showToast("Calcule o frete primeiro", "error");
  ...
  const handleStatusUpdate = async (orderId: string, newStatus: string) => {
      ...
      try {
        ...
        if (newStatus === 'PAGO / EM PRODUÇÃO') {
            updateData.validated_at = new Date().toISOString();
            updateData.validated_by = currentUser?.id;
        }
  ...
  // 6. GERENCIAMENTO DE USUÁRIOS & INICIALIZAÇÃO
  ...
  const handleDeleteUser = async (id: string) => {
      ...
      try {
        ...
        // Should ideally delete from auth.users too via admin API, but client SDK is limited
  ...
  useEffect(() => {
      if (!currentUser) return;
      fetchInitialData();
      const interval = setInterval(() => { fetchOrders(); }, 10000); // Polling for updates
      ...
      <div className="min-h-screen bg-[#09090b] text-zinc-100 p-8">
         ...
         <div className="w-full max-w-[96%] mx-auto flex justify-between items-center mb-10 bg-zinc-900/50 backdrop-blur-md p-6 rounded-[32px] border border-white/5 sticky top-6 z-40 shadow-2xl">
            <div className="flex items-center gap-4">
               <Logo />
               <h1 className="text-xs font-black uppercase text-zinc-400 tracking-widest">Painel Gestor</h1>
            </div>
         ...
         <div className="w-full max-w-[96%] mx-auto bg-zinc-950 border border-white/5 p-1 rounded-2xl flex gap-1 mb-10 overflow-x-auto">
             {['products', 'pending', 'history', 'users'].map(tab => (
                 ...
                 >
                    {tab === 'products' ? 'Produtos' : tab === 'pending' ? 'Pendentes' : tab === 'history' ? 'Acessos' : 'Usuários'}
                 </button>
             ))}
         </div>
                    ...
                    <div className="mb-10">
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                           ...
                           <div className="bg-zinc-900/30 border border-white/5 rounded-[40px] p-10 backdrop-blur-sm">
                               ...
                               <p className="text-[10px] font-black uppercase text-zinc-500 mb-2">Pedidos Totais</p>
                                    ...
                                    <div className="flex gap-3 flex-wrap">
                                        {SIZES_OPTIONS.map(s => (
                                            <button 
                                                key={s}
                                                type="button"
                                                onClick={() => toggleSize(s as Size)}
                                                className={`py-4 px-6 rounded-2xl text-xs font-black border transition-all ${newProduct.available_sizes.includes(s as Size) ? 'bg-zinc-800 border-zinc-600 text-white shadow-lg' : 'bg-zinc-900 border-white/5 text-zinc-500 hover:bg-zinc-800'}`}
                                            >
                                                {s}
                                            </button>
                                        ))}
                            ...
                            <div key={p.id} className="bg-zinc-900/30 border border-white/5 p-6 rounded-[32px] flex items-center gap-6 group hover:border-white/20 transition-all hover:bg-zinc-900/50">
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-3">
                                        ...
                                        <button onClick={() => handleDeleteProduct(p.id)} className="px-4 py-2 bg-rose-950/30 rounded-lg text-[10px] text-rose-500 font-bold uppercase tracking-wider transition-colors border border-rose-500/10 hover:bg-rose-900/50">Excluir</button>
                                    </div>
                                </div>
                            </div>
                        ))}
                                 ...
                                 <div key={order.id} className="bg-zinc-900/30 border border-white/5 rounded-[40px] p-10 backdrop-blur-sm">
                                     <div className="flex justify-between items-start mb-8">
                                         <div>
                                             ...
                                             <p className="text-xs text-zinc-500 uppercase tracking-widest flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-yellow-500 animate-pulse"></span> {new Date(order.created_at).toLocaleDateString()} • {order.user_email}</p>
                                         </div>
                      ...
                      {order.items.map((item, i) => (
                        <div key={i} className="flex justify-between text-xs">
                          <span className="text-zinc-300 font-bold">
                            <span className="bg-zinc-800 px-2 py-0.5 rounded text-zinc-500 mr-2">{item.quantity}x</span> 
                ...
                <div className="md:w-1/2 p-12 md:p-16 flex flex-col justify-between overflow-y-auto max-h-[90vh] custom-scrollbar">
                  <div>
                    <div className="flex justify-between items-start mb-10">
                      ...
                      <button onClick={() => setDetailsModalOpenId(null)} className="p-4 bg-zinc-900 rounded-full text-zinc-500 hover:text-white hover:bg-zinc-800 transition-colors">
                        <X/>
                      </button>
                    </div>
      ...
      {isPaymentOpen && (
        <div className="fixed inset-0 z-[200] bg-black/90 flex items-center justify-center p-6 backdrop-blur-xl">
          <div className="bg-zinc-950 border border-white/10 rounded-[48px] max-w-md w-full p-12 text-center relative overflow-hidden shadow-2xl">
            ...
            <button onClick={() => { navigator.clipboard.writeText(getPixCode()); showToast("Copiado!"); }} className="w-full bg-zinc-900 text-zinc-300 py-5 rounded-2xl font-bold uppercase text-xs tracking-[0.15em] hover:text-white mb-4 flex items-center justify-center gap-3 border border-white/5 hover:bg-zinc-800 transition-colors">
