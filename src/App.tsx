...
export default function App() {
  ...
  useEffect(() => {
    const initAuth = async () => {
      ...
      if (session) {
        ...
        if (profile) {
          // CORREÇÃO CRÍTICA: || '' previne o erro de 'toUpperCase' em null
          setCurrentUser({ 
              id: session.user.id, 
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // CORREÇÃO CRÍTICA: Garantir que valores nulos sejam strings vazias
      setCurrentUser({ 
          id: data.user.id, 
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Cria o usuário na Autenticação COM METADADOS (Isso conserta o problema do banco)
      ...
      if (data.user) {
         // 2. FORÇA A INSERÇÃO NA TABELA PÚBLICA MANUALMENTE (Fallback se o trigger falhar)
         ...
         if (dbError) {
             console.error("Erro ao salvar perfil (fallback):", dbError);
         }

         // 3. LOGIN AUTOMÁTICO IMEDIATO
         // Isso evita ter que fazer login de novo e corrige o problema de "tela preta" se o banco demorar para responder
         setCurrentUser({
            id: data.user.id,
            email: formData.email,
            unit_name: formData.unit_name || '',
      ...
      <div className="min-h-screen bg-[#09090b] text-zinc-100 p-8">
         ...
         <div className="w-full max-w-[96%] mx-auto bg-zinc-950 border border-white/5 p-1 rounded-2xl flex gap-1 mb-10 overflow-x-auto">
             {['products', 'pending', 'history', 'users'].map(tab => (
                 ...
                 >
                    {tab === 'products' ? 'Produtos' : tab === 'pending' ? 'Pendentes' : tab === 'history' ? 'Acessos'}
                 </button>
             ))}
         </div>
               ...
               <div className="flex items-center gap-6">
                   ...
                   <div className="bg-zinc-900/50 border border-white/10 rounded-full px-5 py-2 flex items-center gap-3">
                       ...
                       <span className="text-[11px] font-bold uppercase tracking-widest text-zinc-300">
                           {/* CORREÇÃO CRÍTICA: PROTEÇÃO CONTRA CRASH EM VALORES NULOS */}
